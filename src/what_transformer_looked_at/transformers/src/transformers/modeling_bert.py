...
class BertNormOutput(nn.Module):  # This class is added by Goro Kobayashi
    ...
    def forward(self, hidden_states, attention_probs, value_layer, dense):
        ...
        with torch.no_grad():
            ...
            transformed_shape = (
                transformed_layer.size()
            )  # (batch, seq_length, num_heads, 1, all_head_size)
            transformed_layer = transformed_layer.view(
                transformed_shape[:-2] + (transformed_shape[-1],)
            )
            ...
            weighted_layer = torch.einsum(
                "bhks,bhsd->bhksd", attention_probs, transformed_layer
    ...
    def forward(
        ...
        if output_norms:
            norms_outputs = self.norm(
                hidden_states, self_outputs[1], self_outputs[2], self.output.dense
            )
            outputs = (
                attention_output,
                self_outputs[1],
            ) + norms_outputs  # add attentions and norms if we output them
            return outputs
        # -------------------------------
